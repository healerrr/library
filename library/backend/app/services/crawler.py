...
class CrawlerService:
    ...
    async def preview(self, site: Site) -> dict:
        domains = {site.domain}
        # 使用网站配置的协议（http 或 https）
        site_scheme = getattr(site, 'site_scheme', 'https') or 'https'
        homepage = f"{site_scheme}://{site.domain}/"
    ...
    async def crawl(self, session: AsyncSession, site: Site) -> dict:
        # 回滚前保存后面还需要的普通值，避免 rollback 后 ORM 对象过期
        site_id = int(site.id)
        min_crawl_coverage = float(site.min_crawl_coverage)
        domains = {site.domain}
        maximum = site.crawler_max_pages or self.settings.crawler_max_pages
        allowed_query_params = set(site.allowed_query_params or [])
        errors: list[str] = []
        ...
        async with httpx.AsyncClient(timeout=timeout, headers=headers) as client:
            ...
            while queue and len(visited) < maximum:
                ...
                try:
                    ...
                    # rollback 后显式恢复 ORM 对象，禁止后续属性访问触发隐式 IO
        ...
        # 不要读取可能过期的 site 对象，使用之前保存的 min_crawl_coverage
        prune_blocked = should_block_page_pruning(
            stale_pages=stale_pages,
            coverage=coverage,
            minimum_coverage=min_crawl_coverage,
            errors=errors,
        )
        ...
        # 查询采集记录时不要再访问 crawl_run.id，使用之前保存的 crawl_run_id
