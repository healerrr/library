...
<template>
  <section class="page-wrap">
    ...
    <div v-if="showStrategyModal" class="modal-overlay" @click.self="showStrategyModal = false; editingStrategyId = null">
      <form class="modal strategy-modal" @submit.prevent="saveStrategy">
        ...
        <div class="policy-grid">
          <label><span>只包含这些路由（每行一条，留空表示不限制）</span><textarea v-model="strategyForm.includePatterns" rows="5" placeholder="/about
/news
/products"></textarea></label>
          <label><span>排除这些路由（每行一条）</span><textarea v-model="strategyForm.excludePatterns" rows="5" placeholder="/search
/member
/login"></textarea></label>
    ...
    <div v-if="showPreviewModal && previewResult" class="modal-overlay" @click.self="showPreviewModal = false">
      <div class="modal preview-modal">
        <div class="modal-header">
          ...
          <button class="modal-close" type="button" @click="showPreviewModal = false">×</button>
        </div>
    ...
    <div v-if="showRunsModal" class="modal-overlay" @click.self="showRunsModal = false">
      <div class="modal runs-modal">
        <div class="modal-header">
          <div><span class="eyebrow muted">最近 10 次</span><h2>{{ crawlRunSiteName }} · 采集记录</h2><p>可追踪每次采集覆盖率、过滤数量和旧数据保护状态。</p></div>
          <button class="modal-close" type="button" @click="showRunsModal = false">×</button>
        </div>
    ...
    <div v-if="showAuditModal && auditReport" class="modal-overlay" @click.self="showAuditModal = false">
      <div class="modal audit-modal">
        <div class="modal-header">
          ...
          <button class="modal-close" type="button" @click="showAuditModal = false">×</button>
        </div>
...
.run-list .run-status.protected {
  background: #fff3cd;
  color: #856404;
}

.run-list .run-status.completed {
  background: #d4edda;
  color: #155724;
}

.run-list .run-status.running {
  background: #cce5ff;
  color: #004085;
}
...
.audit-summary .high strong {
  color: #dc3545;
}

.audit-summary .medium strong {
  color: #ffc107;
}
...
.audit-findings .risk-tag.high {
  background: #f8d7da;
  color: #721c24;
}

.audit-findings .risk-tag.medium {
  background: #fff3cd;
  color: #856404;
}

.audit-findings .risk-tag.low {
  background: #d1ecf1;
  color: #0c5460;
}
